...
Most adapter authors only need `useFieldProps` (for any field component) and `useListState` (for custom list implementations). The other hooks are used internally by `useFieldProps` and are only needed if you're building something outside the standard field pattern.
...
**Returns:** `ResolvedCommonProps` plus the component-specific resolved fields. See [Custom components](/enforma/guides/custom-components) for full usage.
