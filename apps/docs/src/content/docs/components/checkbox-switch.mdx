...
See [CommonProps](/enforma/reference/common-props) for `bind`, `label`, `disabled`, `hidden`, `removed`, `validate`, `required`, `description`.
...
<SwitchLabelDemo client:only="react" />
...
<RequiredDemo client:only="react" />
